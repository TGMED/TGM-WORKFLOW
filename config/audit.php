...
return [
    ...
    */
    ...
    // Set here rather than per model, so a column added to any audited model
    // cannot leak a secret into a table a wider audience can read.
    'exclude' => [
        'password',
        'remember_token',
    ],
    ...
    */
    ...
    // On, against the package default. A change made by a scheduled job or an
    // artisan command is still a change somebody has to be able to trace, and
    // leaving it off would put exactly the unattended writes nobody watches
    // outside the trail. Migrations write through the query builder, so they
    // raise no model events and add no noise here.
